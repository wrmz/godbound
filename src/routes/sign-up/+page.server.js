...
export const actions = {
	default: async ({ cookies, request }) => {
		...
		const isUsernameValid =
			username.length >= 3 && username.length <= 30 && regexUsername.test(username);
		...
		if (!isPasswordValid || !isEmailValid || !isUsernameValid) {
			return invalid(401, {
				invalid: true,
				email: !isEmailValid
					? 'Email is not a valid email address or contains an invalid character.'
					: '',
				password: !isPasswordValid ? 'Password must contain 8 or more characters.' : '',
				username: !isUsernameValid
					? 'Username must container 3 or more characters and no special characters.'
					: ''
			});
		}
