	...
	code: (source, language, current) => {
		...
		source = source
			...
			.replace(/^([\-\+])?((?:    )+)/gm, (match, prefix = '', spaces) => {
				...
				// marked replaces tabs with spaces
				let tabs = '';
			...
			.replace(/(\+\+\+|---|:::)/g, (_, delimiter) => {
				return delimeter_substitutes[delimiter];
			})
			.replace(/\*\\\//g, '*/');
		...
		return html
			.replace(/ {13}([^ ][^]+?) {13}/g, (_, content) => {
				return highlighted_spans(content, 'highlight add');
			})
